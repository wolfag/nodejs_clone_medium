...
export function getAllCommentInTheArticle({idArticle, offset, first}) {
  return new Promise (resolve => {
    commentModel
      .find ({idArticle}, async (err, allComment) => {
        if (err) {
          resolve (err);
        }
        let data = allComment
          .filter (comment => !comment.idReply)
          .reverse ()
          .slice (first, first + offset);
        const allreplyComment = allComment.filter (
